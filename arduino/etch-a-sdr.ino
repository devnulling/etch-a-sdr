...
int pinRightButton1 = 8; // up 
int pinRightButton2 = 9; // down
int pinRightButton3 = 7; // mode - black

int pinLeftButton1 = 5; // up
int pinLeftButton2 = 6; // down
int pintLeftButtonMode = 4; // mode - black

int ledPowerButton = 15; // power button led
...
void setup() {
    ...
    pinMode(pinRightKnob, INPUT_PULLUP); // right knob button
    pinMode(pinLeftKnob, INPUT_PULLUP); // left knob button

    pinMode(pinRightButton1, INPUT_PULLUP); // 
    pinMode(pinRightButton2, INPUT_PULLUP); // 
    pinMode(pinRightButton3, INPUT_PULLUP); // 

    pinMode(pinLeftButton1, INPUT_PULLUP); // 
    pinMode(pinLeftButton2, INPUT_PULLUP); // 
    pinMode(pintLeftButtonMode, INPUT_PULLUP);

    pinMode(ledPowerButton, OUTPUT);

    digitalWrite(ledPowerButton, HIGH);
    delay(1000);
    digitalWrite(ledPowerButton, LOW);
    delay(1000);
    digitalWrite(ledPowerButton, HIGH);
    delay(1000);
    digitalWrite(ledPowerButton, LOW);
...
void loop() {
    ...
    if (digitalRead(pinRightKnob) == LOW) {
        Serial.println("R0");
    }
    if (digitalRead(pinLeftKnob) == LOW) {
        Serial.println("L0");
    }
    if (digitalRead(pinRightButton1) == LOW) {
        Serial.println("R1");
    }
    if (digitalRead(pinRightButton2) == LOW) {
        Serial.println("R2");
    }
    if (digitalRead(pinRightButton3) == LOW) {
        Serial.println("R3");
    }
    if (digitalRead(pinLeftButton1) == LOW) {
        Serial.println("L1");
    }
    if (digitalRead(pinLeftButton2) == LOW) {
        Serial.println("L2");
    }
    if (digitalRead(pintLeftButtonMode) == LOW) {
        Serial.println("L3");
    }
